...
class BaseTrainer(ABC):
    ...
    def setup_training_arguments(self) -> TrainingArguments:
        ...
        if dataloader_num_workers > 1:
            try:
                import psutil
                # Check if psutil has the required function before using it
                if hasattr(psutil, 'virtual_memory'):
                    ...
                    # psutil doesn't have virtual_memory function
                    adjusted_workers = 1
                    print(f"psutil.virtual_memory() not available. Reducing dataloader workers to {adjusted_workers} for stability.")
            except (ImportError, AttributeError) as e:
                # If we can't import psutil or access virtual_memory, default to safe setting
                adjusted_workers = 1
                print(f"Unable to check system memory (psutil unavailable: {e}). Reducing dataloader workers to {adjusted_workers} for stability.")
            except Exception as e:
                # Catch any other unexpected errors
                adjusted_workers = 1
                print(f"Unable to check system memory (unexpected error: {e}). Reducing dataloader workers to {adjusted_workers} for stability.")
